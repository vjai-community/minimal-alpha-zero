...
class MnkState(State):
    ...
    def __str__(self):
        board_str = ""
        for y, row in enumerate(reversed(self.board)):  # Print rows from bottom to top
...
class MnkModel(nnx.Module, Model):
    ...
    def __init__(self, m: int, n: int, rngs: nnx.Rngs):
        self.conv = nnx.Conv(self.INPUT_CHANNEL, 4, kernel_size=3, rngs=rngs)
        self.linear = nnx.Linear(4 * m * n, 128, rngs=rngs)
...
class MnkNetwork(Network):
    ...
    def train_and_evaluate(self, replay_buffer: ReplayBuffer, game: MnkGame):
        ...
        for _ in range(self.config.epochs_num):
            ...
            _train_one_epoch(candidate_model, optimizer, metric, data_list, self.config.batch_size)
...
def _train_one_epoch(
    ...
    def loss_fn(model: MnkModel, data_batch: tuple[Array, Array, Array]) -> tuple[float, Array, Array]:
        ...
        loss = probabilities_loss + value_loss
    ...
    for data_batch in [data_list[i : i + batch_size] for i in range(0, len(data_list), batch_size)]:
        boards = jnp.array([b.board_data for (b, _, _) in data_batch])

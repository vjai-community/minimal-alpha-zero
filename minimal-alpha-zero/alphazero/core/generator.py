...
def play(
    ...
    """
    Play a single game with multiple players.
    Each player (model) takes turns in order based on its index in the list.
    Each move is selected after executing a specific number of MCTS simulations, guided by the current best model.
    """
    ...
    while True:
        ...
        if model_config.should_execute_mcts:
            ...
            # NOTE: I simply use the same formula as in the case where MCTS execution is enabled,
            # although there may be a better approach.
            scaled_prior_probs = {
...
def _expand(node: Node, game: Game, model: Model):
    ...
    if reward is not None:
        ...
        # Ref: `Game.receive_reward_if_terminal` method.
